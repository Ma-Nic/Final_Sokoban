...
public class SokobanController {
    ...
    public void setLevelNum(Label newLvl)
    {
    	LevelNum = newLvl;
    }
    ...
    public void setMoveNum(Label newMove)
    {
    	MoveNum = newMove;
    }
    ...
    public void setImage(MapElement[][] map) {
        ...
        LevelArea.setHgap(0); 		//horizontal gap in pixels
        LevelArea.setVgap(0);		//vertical gap in pixels
        //LevelArea.setPadding(new Insets(10, 10, 10, 10)); 	// Padding around the whole grid                
        
        while (map[x][y] != null) {
            if (map[x][y] != null) {
                Image imageFile = new Image(new File (map[x][y].filePath()).toURI().toString(), 32, 32, true, true);
                LevelArea.add(new ImageView(imageFile), x, y);
            }
            x++;
            if (map[x][y] == null) {
                y++;
